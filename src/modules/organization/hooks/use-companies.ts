...
import { companyService, type CompanyListFilter } from "../services/company-service";
...
export function companiesListQueryKey(membership: CompanyListFilter = "active") {
  return [...companiesQueryKey, membership] as const;
}
...
export function useCompanies(membership: CompanyListFilter = "active") {
  return useQuery({
    queryKey: companiesListQueryKey(membership),
    queryFn: () => companyService.list(membership),
...
export function useRestoreCompany() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (companyId: string) => companyService.restore(companyId),
    onSuccess: (data) => {
      void qc.invalidateQueries({ queryKey: companiesQueryKey });
      if (data?.company_id) {
        qc.setQueryData(companyQueryKey(data.company_id), data);
