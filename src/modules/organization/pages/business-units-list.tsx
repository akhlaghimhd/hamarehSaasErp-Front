/**
 * FE-ORG — فهرست واحدهای کسب‌وکار (هم‌تراز شعب/واحدها از نظر UX)
 */
"use client";
...
import {
  ArrowDown, ArrowUp, ArrowUpDown, CircleHelp, Columns3, Layers, Link2, Loader2, Pencil, Plus,
...
type AssignForm = { company_id: string; is_primary: boolean };
...
export function BusinessUnitsListPage() {
  ...
  const assignForm = useForm<AssignForm>({ defaultValues: { company_id: "", is_primary: false } });
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === "name") {
        ...
        av = a.company_assignments?.length ?? 0;
        bv = b.company_assignments?.length ?? 0;
      } else {
      ...
      for (const t of targets) {
        try {
          if (kind === "activate" || kind === "deactivate") {
            ...
            await businessUnitService.softDelete(t.business_unit_id);
          } else {
            await businessUnitService.restore(t.business_unit_id);
          }
          ok += 1;
        } catch { /* continue */ }
      }
  ...
  const allPageSelected =
    pageRows.length > 0 && pageRows.every((r) => selected.has(r.business_unit_id));
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          description="بخش‌بندی مدیریتی برای گزارش و کنترل عملکرد"
          breadcrumbs={[
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[200px] flex-1">
            ...
            <Input
              className="h-9 ps-9"
              placeholder="جستجو در نام، کد، توضیح…"
              value={search}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-9">
                <Columns3 className="h-4 w-4" /> ستون‌ها
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              ...
              {COLS.map((c) => (
                ...
                >
                  <Checkbox checked={visibleCols.has(c.id)} className="ms-0 me-2" />
                  {c.label}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
        ...
        {selected.size > 0 && canManage ? (
          <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-muted/40 px-3 py-2 text-sm">
            ...
            {membership === "active" ? (
              <>
                <Button size="sm" variant="outline" className="h-8" disabled={busy} onClick={() => requestBulk("activate", rows.filter((r) => selected.has(r.business_unit_id) && r.is_active === false))}>
              ...
              <Button size="sm" variant="outline" className="h-8" disabled={busy} onClick={() => requestBulk("restore", rows.filter((r) => selected.has(r.business_unit_id)))}>
                <RotateCcw className="h-3.5 w-3.5" /> بازگردانی گروهی
              </Button>
            )}
            <Button size="sm" variant="ghost" className="h-8 ms-auto" onClick={() => setSelected(new Set())}>
        ...
        <div className={cn("rounded-xl border bg-card", listQuery.isFetching && listQuery.data ? "opacity-70 transition-opacity" : "")}>
          {showSkeleton ? (
            ...
            <div className="p-6 text-sm text-destructive">
              ...
              <Button variant="outline" size="sm" className="ms-2" onClick={() => void listQuery.refetch()}>
                تلاش مجدد
              </Button>
            </div>
          ) : filtered.length === 0 ? (
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(v) => {
                        setSelected((prev) => {
                          const next = new Set(prev);
                          pageRows.forEach((r) => {
                            if (v) next.add(r.business_unit_id);
                            else next.delete(r.business_unit_id);
                          });
                          return next;
                        });
                      }}
                ...
                {pageRows.map((r) => {
                  ...
                  return (
                    <TableRow key={r.business_unit_id} data-state={selected.has(r.business_unit_id) ? "selected" : undefined}>
                      <TableCell>
                        <Checkbox
                          checked={selected.has(r.business_unit_id)}
                          onCheckedChange={(v) => {
                            setSelected((prev) => {
                              const next = new Set(prev);
                              if (v) next.add(r.business_unit_id);
                      ...
                      {visibleCols.has("name") ? (
                        <TableCell>
                          ...
                          {r.description ? (
                            <div className="mt-0.5 max-w-xs truncate text-xs text-muted-foreground">{r.description}</div>
                          ) : null}
                        </TableCell>
                      ) : null}
                      {visibleCols.has("code") ? (
                        <TableCell>
                          <span className="font-mono text-xs" dir={codeDisp.dir}>{codeDisp.text}</span>
                        </TableCell>
                      ) : null}
                      ...
                      {visibleCols.has("created") ? (
                        <TableCell className="text-sm text-muted-foreground">{fd(r.created_at)}</TableCell>
                      ) : null}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-0.5">
                            {membership === "active" && canManage ? (
                              <>
                                <IconAction label="ویرایش" onClick={() => openEdit(r)}>
                                  <Pencil className="h-3.5 w-3.5" />
                                </IconAction>
                                <IconAction label="اتصال شرکت" onClick={() => openAssign(r)}>
                                  <Link2 className="h-3.5 w-3.5" />
                                </IconAction>
                                {r.is_active !== false ? (
                                  <IconAction label="غیرفعال" onClick={() => requestBulk("deactivate", [r])}>
                                    <PowerOff className="h-3.5 w-3.5" />
                                  </IconAction>
                                ) : (
                                  <IconAction label="فعال" onClick={() => requestBulk("activate", [r])}>
                                    <Power className="h-3.5 w-3.5" />
                                  </IconAction>
                                )}
                                <IconAction label="حذف" variant="destructive" onClick={() => requestBulk("delete", [r])}>
                                  <Trash2 className="h-3.5 w-3.5" />
                                </IconAction>
                              </>
                            ) : null}
                            {membership === "deleted" && canManage ? (
                              <IconAction label="بازگردانی" onClick={() => requestBulk("restore", [r])}>
        ...
        {filtered.length > PAGE_SIZE ? (
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            <span>
              {toFaDigits((page - 1) * PAGE_SIZE + 1)}–{toFaDigits(Math.min(page * PAGE_SIZE, filtered.length))} از {toFaDigits(filtered.length)}
            </span>
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetContent side="right" className="flex w-full flex-col sm:max-w-lg">
            <SheetHeader>
              <SheetTitle>{editing ? "ویرایش واحد کسب‌وکار" : "واحد کسب‌وکار جدید"}</SheetTitle>
            </SheetHeader>
            <form className="flex flex-1 flex-col" onSubmit={form.handleSubmit(submitForm)}>
              <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                ...
                <div className="space-y-3 rounded-lg border p-3">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <button type="button" className="flex items-center gap-1.5 text-sm" onClick={() => form.setValue("is_active", !form.watch("is_active"))}>
                      فعال
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <CircleHelp className="h-3.5 w-3.5 text-muted-foreground" onClick={(e) => e.stopPropagation()} />
                        </TooltipTrigger>
                        <TooltipContent className="max-w-xs">در گزارش و انتخاب‌های عملیاتی قابل استفاده باشد یا نه</TooltipContent>
                      </Tooltip>
                    </button>
              ...
              <SheetFooter className="gap-2 border-t px-5 py-3">
                ...
                <Button type="submit" disabled={busy || !canManage}>
                  {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : editing ? "ذخیره" : "ثبت"}
                </Button>
              </SheetFooter>
            </form>
          </SheetContent>
          ...
          <SheetContent side="right" className="flex w-full flex-col sm:max-w-md">
            ...
            <form className="flex flex-1 flex-col" onSubmit={assignForm.handleSubmit(submitAssign)}>
              ...
              <SheetFooter className="gap-2 border-t px-5 py-3">
                ...
                <Button type="submit" disabled={busy || !canManage}>
                  {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "اتصال"}
                </Button>
              </SheetFooter>
            </form>
          </SheetContent>
