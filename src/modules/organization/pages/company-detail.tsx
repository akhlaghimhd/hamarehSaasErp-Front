    ...
    <div className="space-y-6">
      <PageHeader
        ...
        description={
          <span className="flex flex-wrap items-center gap-2 text-xs">
            ...
            {isDeleted ? <StatusChip label="حذف‌شده" tone="danger" /> : null}
      ...
      {isDeleted ? (
        <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm">
          <p className="font-medium text-amber-900 dark:text-amber-200">این شرکت حذف شده است</p>
          <p className="mt-1 text-xs text-muted-foreground">
            مشاهده اطلاعات مجاز است؛ ثبت شعبه، واحد یا ویرایش تا زمان بازگردانی غیرفعال است. پس از بازگردانی، شرکت غیرفعال می‌ماند تا خودتان فعال کنید.
          </p>
          <div className="mt-3">
        ...
        <DialogContent>
          ...
          <form
            ...
            onSubmit={deptForm.handleSubmit(async (values) => {
              if (!values.branch_id) { toast.error("انتخاب شعبه الزامی است."); return; }
              try {
          ...
          >
            <div className="space-y-1.5">
              <Label>شعبه</Label>
              <select className="flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm" {...deptForm.register("branch_id")}>
