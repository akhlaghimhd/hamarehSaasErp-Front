...
export function BranchesListPage() {
  ...
  }, [allRows, companyFilter, statusFilter, query, sortKey, sortDir]);
  ...
  const isFiltered =
    query.trim().length > 0 || companyFilter !== ALL || statusFilter !== "all";
  ...
  const invalidateAll = () => {
    void qc.invalidateQueries({ queryKey: ["organization", "companies"] });
  };
      ...
      <div className="flex min-h-0 flex-col gap-3">
        ...
        <div className="min-h-0 flex-1 overflow-auto rounded-xl border border-border">
          {isLoading ? (
            ...
            <EmptyState
              icon={GitBranch}
              title={isFiltered ? "نتیجه‌ای پیدا نشد" : "شعبه‌ای ثبت نشده"}
              description={
                isFiltered
                  ? "عبارت جستجو یا فیلتر را تغییر دهید."
                  : "اولین شعبه را برای یکی از شرکت‌ها ثبت کنید."
              }
            />
          ) : (
            <Table>
              ...
              <TableBody>
                {pageRows.map((row) => (
                  <TableRow
                    key={row.branch_id}
                    data-state={selected.has(row.branch_id) ? "selected" : undefined}
                  >
                    ...
                    {visible.company !== false ? (
                      <TableCell className="px-2 text-xs">
                        <Link href={companyDetailPath(row.company_id)} className="hover:underline">
                          {row.company_name}
                        </Link>
                      </TableCell>
                    ) : null}
