...
export function CompaniesListPage() {
  ...
  const formFields = (excludeId?: string) => (
      ...
      <div className="flex min-h-0 flex-col gap-3">
        ...
        {selected.size > 0 || bulkBusy ? (
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border/80 bg-muted/30 px-3 py-2 text-sm">
            ...
            {canUpdate && !bulkBusy ? (
              <>
                <Button type="button" size="sm" variant="outline" className="h-7" onClick={() => void runBulk("activate", selectedRows)}>
                  فعال‌سازی
                </Button>
                <Button
                  type="button"
                  ...
                  className="h-7"
                  onClick={() => {
                    if (window.confirm("شرکت‌های انتخاب‌شده (غیر از اصلی) و زیرمجموعه‌هایشان غیرفعال شوند؟"))
            ...
            {canDelete && !bulkBusy ? (
              <Button
                ...
                onClick={() => {
                  if (window.confirm("حذف نرم موارد انتخاب‌شده؟")) void runBulk("delete", selectedRows);
                }}
              >
                حذف
              </Button>
            ...
            {bulkBusy ? (
              <Button type="button" size="sm" variant="ghost" className="h-7" onClick={() => { cancelRef.current = true; }}>
                توقف
              </Button>
            ) : (
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description={isFiltered ? "عبارت یا فیلتر را تغییر دهید." : "اولین شرکت سازمان را ثبت کنید."}
          />
        ) : (
              ...
              <TableBody>
                {pageRows.map((row) => (
                  <TableRow key={row.company_id} className="[&_td]:py-1.5">
                    ...
                    {visible.name ? (
                      <TableCell className="px-2">
                        <div className="flex flex-col gap-0.5">
                          ...
                          {row.is_primary ? (
                            <span className="text-[10px] text-amber-700 dark:text-amber-400">شرکت اصلی</span>
                          ) : null}
                        </div>
                      </TableCell>
                    ) : null}
                    {visible.code ? (
                      <TableCell className="px-2">
                        <span className="font-mono text-xs" dir="ltr">{row.code}</span>
                      </TableCell>
                    ) : null}
                    {visible.kind ? (
                    ...
                    {visible.parent ? (
                      <TableCell className="px-2 text-xs">
                        {row.parent_company_id ? parentMap.get(row.parent_company_id) ?? "—" : "—"}
                      </TableCell>
                    ) : null}
                    {visible.branches ? (
                      ...
                      <TableCell className="px-2">
                        <div className="flex items-center gap-0.5">
                          ...
                          {canUpdate && row.is_active !== false ? (
                            <IconAction
                              label={row.is_primary ? "شرکت اصلی قابل غیرفعال نیست" : "غیرفعال‌سازی"}
