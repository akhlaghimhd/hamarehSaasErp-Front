...
export function MemberDetailPage() {
  ...
  const setAvatarPreviewSafe = useCallback((url: string | null) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!userId) {
        setAvatarPreviewSafe(null);
        return;
      }
      const has =
        Boolean((profile as { has_avatar?: boolean } | null | undefined)?.has_avatar) ||
        Boolean((profile as { avatar_url?: string | null } | null | undefined)?.avatar_url);
      if (!has) {
        setAvatarPreviewSafe(null);
        return;
      }
      const url = await profileService.fetchAvatarObjectUrlForUser(userId);
      if (!cancelled) setAvatarPreviewSafe(url);
    };
    void load();
    return () => {
      cancelled = true;
  ...
  }, [userId, profile, setAvatarPreviewSafe]);
  ...
  const bioText =
    (
      (profile as { display_bio?: string | null; description?: string | null; bio?: string | null } | null | undefined)
        ?.display_bio ??
      (profile as { description?: string | null } | null | undefined)?.description ??
      (profile as { bio?: string | null } | null | undefined)?.bio ??
      ""
    )?.trim() ?? "";
          ...
          <CardContent>
            {editingIdentity ? (
              <div className="space-y-4">
                ...
                {canManageOwner ? (
                  <label className="flex items-start gap-3 rounded-md border border-border/60 px-3 py-3">
                    <Checkbox
                      className="mt-0.5"
                      checked={isOwner}
                    ...
                    <span className="space-y-0.5 text-sm">
                      <span className="block font-medium">مدیر اصلی سازمان</span>
                      <span className="block text-xs text-muted-foreground">
                        فقط مالک فعلی می‌تواند این گزینه را تغییر دهد.
                      </span>
                    </span>
                  </label>
                ) : null}
              ...
              <div className="grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <FieldLine label="نام خانوادگی" value={data.user?.last_name ?? ""} />
                <FieldLine label="موبایل" value={data.user?.mobile ?? ""} dir="ltr" />
                <div className="sm:col-span-2">
                  <FieldLine label="ایمیل" value={data.user?.email ?? ""} dir="ltr" />
                </div>
                <FieldLine label="عضویت از" value={formatDate(data.joined_at)} />
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Dialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              کاربر از فهرست جاری سازمان خارج می‌شود. این عمل soft-delete است و
              از بخش حذف‌شده‌ها قابل بازگردانی است.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
