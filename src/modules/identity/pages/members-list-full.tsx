...
function activityOf(r: TenantUserDto, deletedView: boolean): ActivityKind {
  if (deletedView && r.deleted_at && isRecent(r.deleted_at)) return "removed";
  if (!deletedView && r.created_at && isRecent(r.created_at, 3)) return "new";
  if (!deletedView && r.updated_at && isRecent(r.updated_at)) {
    if (Number(r.status) === 1 && r.created_at) {
      const c = new Date(r.created_at).getTime();
      const u = new Date(r.updated_at).getTime();
      if (u - c > 60_000) return "activated";
    }
...
const ACTIVITY_LABEL: Record<Exclude<ActivityKind, null>, string> = {
  new: "جدید",
  activated: "تازه‌فعال",
  deactivated: "تازه‌غیرفعال",
  removed: "تازه‌حذف",
  restored: "تازه‌بازگشت",
};
...
function mapImportRows(table: string[][]): ImportRow[] {
  ...
  for (let r = 1; r < table.length; r++) {
    ...
    const first =
      (iFirst >= 0 ? row[iFirst] : row[0] ?? "").trim() || "کاربر";
    ...
    const mobile = (iMobile >= 0 ? row[iMobile] : "").trim() || undefined;
    const password =
      (iPass >= 0 ? row[iPass] : "").trim() ||
      `Tmp${Math.random().toString(36).slice(2, 10)}!1`;
    out.push({
      first_name: first,
      last_name: last,
      email,
...
export function MembersListPage() {
  ...
  const onImportFile = async (file: File) => {
    ...
    try {
      ...
      if (!mapped.length) {
        toast.error(
          "در فایل ردیف معتبری یافت نشد. ستون‌ها: نام، نام خانوادگی، ایمیل، موبایل (اختیاری)"
        );
        return;
      }
      cancelRef.current = false;
      setBulkBusy(true);
      setBulkProgress({ done: 0, total: mapped.length });
      let ok = 0;
      let fail = 0;
      for (let i = 0; i < mapped.length; i++) {
        ...
        setBulkProgress({ done: i + 1, total: mapped.length });
      }
      setBulkBusy(false);
      if (ok) toast.success(`${toFaDigits(ok)} کاربر از فایل افزوده شد`);
      if (fail)
        toast.error(
          `${toFaDigits(fail)} ردیف وارد نشد (ایمیل تکراری یا داده نامعتبر)`
        );
    } catch {
      toast.error("خواندن فایل ممکن نشد. فایل را به‌صورت CSV ذخیره کنید.");
    } finally {
      setImportBusy(false);
  ...
  const downloadTemplate = () => {
    const html =
      '\uFEFF<html><head><meta charset="utf-8" /></head><body dir="rtl"><table border="1"><tr><th>نام</th><th>نام خانوادگی</th><th>ایمیل</th><th>موبایل</th></tr><tr><td>علی</td><td>رضایی</td><td>ali.rezaei@example.com</td><td>09121234567</td></tr></table></body></html>';
    ...
    a.href = url;
    a.download = "olgu-karbaran.xls";
    a.click();
    URL.revokeObjectURL(url);
    toast.message(
      "الگوی اکسل دانلود شد — پس از پر کردن، از «ورود از اکسل» استفاده کنید"
    );
  };
      ...
      <div className="flex min-h-0 flex-col gap-3">
              ...
              {canCreate ? (
                <>
                  <input
                    ref={importRef}
                    type="file"
                    accept=".csv,.txt,.xls,.xlsx,text/csv,text/plain"
                    className="hidden"
                    onChange={(e) => {
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(v) =>
              setMembershipFilter(v as MembershipListFilter)
            }
          >
              ...
              <>
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!targets.length) {
                      toast.message("کاربر غیرفعالی در انتخاب نیست");
                      return;
                    }
                    setConfirm({
                      kind: "activate",
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!targets.length) {
                      toast.message(
                        "کاربر فعالی (به‌جز خودتان) در انتخاب نیست"
                      );
                      return;
                    }
                    setConfirm({
            ...
            {isDeletedView && canRestore ? (
              <Button
                ...
                onClick={() =>
                  setConfirm({
                    kind: "restore",
                    count: selectedRowsOrdered.length,
                    targets: selectedRowsOrdered,
                  })
                }
              >
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border bg-card shadow-[var(--shadow-xs)]">
          <div
            ...
            style={{ maxHeight: "min(62vh, calc(100dvh - 14.5rem))" }}
          >
            <Table>
              <TableHeader className="sticky top-0 z-20 bg-card/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-card/80">
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="w-12 px-2 text-center text-xs">
                    ردیف
                  </TableHead>
                  {COLS.map((c) => {
                    if (c.id !== "name" && c.id !== "actions" && !visible[c.id])
                      return null;
                    ...
                    return (
                      <TableHead
                        ...
                        className={cn(
                          ...
                          c.id === "actions" && "w-[7.5rem] text-center"
                        )}
                      >
                        {sortable && c.sort ? (
                          <button
                      ...
                      <EmptyState
                        ...
                        title={
                          isFiltered
                            ...
                            : isDeletedView
                              ? "حذف‌شده‌ای نیست"
                              : "هنوز کاربری افزوده نشده"
                        }
                        description={
                          isFiltered
                            ? "عبارت جستجو یا فیلتر را تغییر دهید"
                            : isDeletedView
                              ? "کاربران حذف‌شده از اینجا قابل بازگردانی هستند"
                              : "با دکمه افزودن یا ورود از اکسل شروع کنید"
                        }
                      />
                    </TableCell>
                  </TableRow>
                  ...
                  pageRows.map((row, idx) => {
                    ...
                    return (
                      <TableRow
                        ...
                        className="h-9 transition-colors hover:bg-muted/50 data-[state=selected]:bg-primary/5"
                      >
                          ...
                          <TableCell className="max-w-[14rem] px-2 py-1">
                            <div className="flex min-w-0 items-center gap-1.5">
                              ...
                              {row.is_owner ? (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <span className="inline-flex shrink-0 items-center justify-center rounded-full bg-amber-500/15 p-0.5 text-amber-700 dark:text-amber-300">
                        ...
                        {visible.email ? (
                          <TableCell className="max-w-[11rem] truncate px-2 py-1 text-xs text-muted-foreground">
                            {row.user?.email ?? "—"}
                          </TableCell>
                        ) : null}
                        {visible.mobile ? (
                        ...
                        {visible.lastChange ? (
                          <TableCell className="px-2 py-1 text-xs tabular-nums text-muted-foreground">
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span>{fd(lastChangeAt(row))}</span>
                              </TooltipTrigger>
                              <TooltipContent>
                                {fdt(lastChangeAt(row))}
                              </TooltipContent>
                            </Tooltip>
                          </TableCell>
                        ) : null}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t bg-muted/20 px-3 py-2 text-xs">
            <div className="tabular-nums text-muted-foreground">
              {total === 0
                ? "موردی نیست"
                : `نمایش ${toFaDigits((safePage - 1) * pageSize + 1)} تا ${toFaDigits(Math.min(safePage * pageSize, total))} از ${toFaDigits(total)}`}
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Select
