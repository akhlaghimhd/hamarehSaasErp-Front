...
import {
  ...
  mapImportRows, IconAction, ActivityBadge, highestRoleName,
                      ...
                      <TableRow key={row.tenant_user_id} data-state={selected.has(row.tenant_user_id) ? "selected" : undefined} className="h-9 transition-colors hover:bg-muted/50 data-[state=selected]:bg-primary/5">
                        ...
                        <TableCell className="px-1 py-1">
                          <div className="flex items-center justify-center gap-0.5">
                            {isDeletedView ? (
                              canRestore ? <IconAction label="بازگردانی به سازمان" onClick={() => void restoreOne(row)} disabled={bulkBusy}><RotateCcw className="h-3.5 w-3.5" /></IconAction> : null
                            ) : (
                              <>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button type="button" variant="ghost" size="icon" className="h-7 w-7 shrink-0" asChild>
                                  <Link href={memberDetailPath(row.tenant_user_id)} aria-label="جزئیات" onClick={(e) => e.stopPropagation()}><Eye className="h-3.5 w-3.5" /></Link>
                                </Button>
                              </TooltipTrigger>
