 ...
 * Controlled identifier (watch + value) to avoid uncontrolled→controlled warning.
 */
...
export default function LoginPage() {
  ...
  // Always a string so the input stays controlled for the lifetime of the component
  ...
  const startOtpSession = (
    mobile: string,
    debug?: string,
    preserveValidityUntil?: number
  ) => {
    const now = Date.now();
    setLastOtpSession({
      mobile,
      validUntil: preserveValidityUntil ?? now + OTP_VALIDITY_SEC * 1000,
  ...
  const onRequestOtp = async () => {
    ...
    // Same-number resume: if still valid, just go to code step
    if (
      lastOtpSession &&
    ...
    const nextCount = otpSendCount + 1;
    if (nextCount >= 2 && !needHumanCheck) {
      setOtpSendCount(nextCount);
    ...
    try {
      ...
      setOtpSendCount(nextCount);
      startOtpSession(mobile, res.debugCode);
      setOtpStep("code");
  ...
  const onVerifyOtp = useCallback(
    ...
    [otpCode, otpMobile, codeValidityLeft]
  );
  ...
  const onSelectOrg = async (orgId: string) => {
    ...
    setFormError(null);
    try {
      await authService.selectOrganization(preAuth, orgId);
      toast.success("ورود با موفقیت انجام شد");
      goToDashboard();
    } catch (err) {
      const raw =
    ...
    <LoginShell>
      <div className="flex w-full flex-1 flex-col justify-center gap-4 p-5 sm:p-6 lg:max-w-[380px]">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            با رمز عبور یا کد یک‌بارمصرف وارد شوید
          </p>
        </div>
        ...
        {needHumanCheck ? (
          ...
          >
            ...
            <div className="pt-2">
              <Button type="submit" className="h-9 w-full text-sm" disabled={otpBusy}>
                {otpBusy ? (
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-1">
              <Button
                ...
                disabled={otpBusy || otpCode.replace(/\D/g, "").length !== OTP_LENGTH}
              >
                {otpBusy ? (
