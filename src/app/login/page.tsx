...
export default function LoginPage() {
  ...
  /** OTP network/verify failures — gate arms after 1 */
  ...
  const onVerifyOtp = useCallback(
    async (codeOverride?: string) => {
      ...
      if ((humanGateArmed || otpFails >= 1) && !humanPassOnceRef.current) {
        setHumanGateArmed(true);
        setPendingAfterCheck("otp-verify");
        setNeedHumanCheck(true);
        return;
      }
      humanPassOnceRef.current = false;
    ...
    [otpCode, otpMobile, codeValidityLeft, blockedUntilEdit, humanGateArmed, otpFails]
  );
